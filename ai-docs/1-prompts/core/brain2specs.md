...
2. Re-read this prompt (stored at `ai-docs/1-prompts/core/brain2specs.md`) so the agent follows the required structure.
3. Provide the transcript plus prompt instructions to the assistant and request `specs<session_number>.md`.
4. Save the generated document to `ai-docs/3-specs/specs<session_number>.md`.
5. Confirm the file includes the Vision, Tasks (with status glyphs), and Development Conventions sections before moving on.
